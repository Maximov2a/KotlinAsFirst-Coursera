 ...
 *
 * Путник двигался
 * t1 часов со скоростью v1 км/час,
 * затем t2 часов — со скоростью v2 км/час
...
fun timeForHalfWay(t1: Double, v1: Double,
    ...
    val s2 = t2*v2
    val s3 = t3*v3
    val s05 : Double = (s1+s2+s3)/2
    if (s1>s05)
        return s05 / v1
        else if ((s2+s1) > s05)
        return (s05-s1) / v2+t1
        else
        return (s05-s1-s2) / v3 +t1+t2
}
...
fun triangleKind(a: Double, b: Double, c: Double) :Int=TODO()/*  {
     if (!((a < (b + c)) && (b < (a + c)) && (c < (a + b)))) return -1
         ...
         val alfa = acos((b * b + c * c - a * a) / 2 * b * c)
         val betta = acos((a * a + c * c - b * b) / 2 * b * c)
         val tetta = 180 - (alfa + betta)

       if ((alfa == 90.0) || (betta == 90.0) || (tetta == 90.0)) return 1
        else if (alfa < 90 && betta < 90 && tetta < 90) return 0
      else  if (alfa > 90 || betta > 90 || tetta > 90) return 2
